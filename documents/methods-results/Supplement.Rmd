...
Effective management of marine systems requires quantitative assessments of ocean health. To assess the ocean condition of Tetiaroa a collaborative effort was undertaken between Tetiaroa Society’s 4site team and the University of California, Santa Barbara’s National Center for Ecological Analysis and Synthesis (NCEAS). The Ocean Health Index (OHI) framework was used to assess Tetiaroa. Tetiaroa is a mostly uninhabited collection of islets in the Windward group of the Society Islands of French Polynesia. The islets are under a 99-year lease contracted by Marlon Brando, and are the location of a small vacation resort, the Brando. Management, conservation, and protection of the entire island are carried out by the Tetiaroa Society, a US-based non-profit organization. Specifically, we assessed the performance of 5 goals: Sense of Place, Conservation Economy, Habitat Services, Tourism and Recreation, and Biodiversity. Using best available data and locally relevant reference points, each goal was scored on a scale of 0-100. Goal scores were then averaged to obtain an overall health score for Tetiaroa. The overall score for Tetiaroa was 79, with goal scores ranging from 48 for the Conservation Economy goal and to 90 for the Tourism and Recreation goal.

The Tetiaroa OHI+ [Github repository](https://github.com/OHI-4site/tet-prep) contains all the data and scripts used to conduct the Tetiaroa assessment. Future researchers can use this resource to build upon this assessment as better data become available and goal models evolve to better assess the status of the marine and terrestrial environments around Tetiaroa. Furthermore, the repository promotes transparent and open research, as well as more streamlined collaboration (Stewart Lowndes *et al.*, 2017).

# Applying the Ocean Health Index (OHI) to Tetiaroa
...
The health of Tetiaroa’s marine and coastal resources are critical to the mission of ensuring “island and coastal communities have a future as rich as their past - strengthening their resilience to global change, enhancing their ecosystem services, and preserving their culture” (https://www.tetiaroasociety.org/). Tetiaroa’s overall OHI score of 79 suggests that marine and coastal resources are supporting this goal fairly well. The goals with the highest scores are Iconic Species (92) and Tourism and Recreation (90). This suggests that many of the species with the most cultural and spiritual value to Tetiaroa are well protected, and efforts to implement sustainable tourism practices are successfully mitigating the impact of this activity on the island's environment. 
...
**Figure 2.1. OHI+ Tetiaroa Results** Each goal and sub-goal of the Tetiaroa Ocean Health Index assessment is represented by a petal. The length of each petal corresponds to the score (0-100), with longer petals indicating higher scores. The color of the petals also correspond to the scores with red indicating lower scores and blue indicating higher scores. Grey petals indicate goals that were included but did not have enough data to calculate a score. The overall index score is shown in the center of the plot.
<br>   

**Table 2.1. Results for each goal and sub-goal in the Tetiaroa OHI+ assessment.** Goal, and sub-goal, scores, as well as current status and predicted future status. Where 'Status' is the current status relative to the reference point; 'Future' is the likely future status based on the trend, pressures, and resilience; and 'Score' is the final goal or sub-goal score reflected in Figure 2.1.  
<br>  
...
<span style="color: #f2cf4a; font-family: Arial, sans-serif; font-size: large;">Score = 80</span>
...
<span style="color: #f2cf4a; font-family: Arial, sans-serif; font-size: large;">Score = 71</span>
...
**Table 3.1.** Description of condition, extent, and trend calculations for habitat data (Note: extent is not used to calculate the habitat subgoal, but is used for the coastal protection and carbon storage goals). More information about the sources used to generate these values is located in Section 6, References.
...
The condition of the native land habitat was calculated by finding the weighted average of the scored land cover categories (Eq 3.2). 

$$
C_{land} = \frac { \displaystyle\sum _{ i=1 }^{ N }{ { (S }_{ i } } \times { A }_{ i }) }{ { { A } }  }, (Eq. 3.2) 
$$  
For every $i$ vegetation type, we multiplied the score $S_{i}$ from Table 3.3, by the area $A_{i}$ of that vegetation type on Tetiaroa. These were divided by the total area $A$ to find the overall native land habitat condition. 
...
<span style="color: #f2cf4a; font-family: Arial, sans-serif; font-size: large;">Score = 89</span>
...
The status of this goal, $x_{cp}$, was calculated to be a function of the relative health of the habitats, $k$, weighted by their area and protectiveness rank (Table 3.6), such that:
...
**Table 3.5. Total area (km^2^) of benthic habitat types in Tetiaroa** Modified from table on page 21 of the LOF French Polynesia Final Report.
...
**Table 3.6. Coastal protectiveness ranks**
...
<span style="color: #f2cf4a; font-family: Arial, sans-serif; font-size: large;">Score = 48</span>
...
**Table 3.7. Ideal yearly budget for Tetiaroa Society Operations**
...
<span style="color: #f2cf4a; font-family: Arial, sans-serif; font-size: large;">Score = 88</span>
...
<span style="color: #f2cf4a; font-family: Arial, sans-serif; font-size: large;">Score = 92</span>
...
After compiling a list of species from this source, conservation statuses were obtained using the RRedlist package and each species was assigned a score based on its conservation status (Table 3.8). 
...
<span style="color: #f2cf4a; font-family: Arial, sans-serif; font-size: large;">Score = 84</span>
...
We computed a score for each zone based on if they meet four criteria considered important for successful management,  (Table 3.9), developed through conversations with Frank Murphy and Neil Davies: 
...
<span style="color: #f2cf4a; font-family: Arial, sans-serif; font-size: large;">Score = 90</span>
...
For this goal, we used sustainability datafrom [SA Beachcomber's 2019 sustainability report](https://en.calameo.com/read/004500545654d0392cc93). We did request actual data from The Brando, contacting the sustainability manager, Alban Perret. He was not able to share any data at this time, as The Brando has had to push their sustainability work to 2021. Alban said to reach out again in Q2 2021 for an update.
...
$$
status_{tr} = \frac { \displaystyle\sum _{ k=1 }^{ N }{ { (S }_{ c} } \times { M }) }{ \displaystyle\sum _{ k=1 }^{ N }{ { c } }  }, (Eq. 3.10)
$$  
...
The pressure score, $p$, describes the cumulative pressures acting on a goal which suppress the goal score.  Pressure scores range from 0 to 1, and they are calculated for each goal and region and include both ecological ($p_{E}$) and social pressures ($p_{S}$) (Table 3.10, Figure 3.1), such that:

$$
{ p }\quad =\quad \gamma *{ p }_{ E }\quad +\quad (1-\gamma )*{ p }_{ S }, (Eq. 3.11)
$$
...
<br>
**Table 3.10. Pressure data and categories**
Description of the stressor data layers used to calculate overall pressure for Tetiaroa.  Each data layer is assigned to an ecological or social category, and ecological data are assigned to one of five subcategories. 
...
Ocean acidification pressure scores are found using the same methods as the OHI Global assessment. Additional information on how this data is processed to obtain 0-1 scores can be found [here](https://cdn.rawgit.com/OHI-Science/ohiprep/master/globalprep/prs_oa/v2017/create_oa_layer.html)(Feely et *al* 2009). The global data is cropped to a 3 nautical mile boundary around Tetiaroa.   

<u>Sea Level Rise</u> 
Sea level rise pressure scores are found using the same methods as the OHI Global assessment. Additional information on how this data is processed to obtain 0-1 scores can be found [here](https://ohi-science.org/ohiprep_v2019/globalprep/prs_slr/v2019/slr_layer_prep_v2.html)(AVISO, 2019). The global data is cropped to a 3 nautical mile boundary around Tetiaroa.  

<u>Sea Surface Temperature</u> 
Sea surface temperature pressure scores are found using the same methods as the OHI Global assessment. Additional information on how this data is processed to obtain 0-1 scores can be found [here](http://ohi-science.org/ohiprep_v2020/globalprep/prs_sst/v2018/sst_layer_prep.html)(Saha et *al* 2018). The global data is cropped to a 3 nautical mile boundary around Tetiaroa.  

<u>Ultraviolet Radiation</u> 
Ultraviolet radiation pressure scores are found using the same methods as the OHI Global assessment. Additional information on how this data is processed to obtain 0-1 scores can be found [here](http://ohi-science.org/ohiprep_v2020/globalprep/prs_uv/v2020/uv_dataprep.html) (Jari et *al*. 2014). The global data is cropped to a 3 nautical mile boundary around Tetiaroa.

**Habitat Destruction**

Habitat destruction from various industries or overcrowding is a significant pressure on many coastal and intertidal environments. On Tetiaroa, this might result from tourists walking in sensitive, shallow intertidal areas, accidentally disturbing coral structures while snorkeling or boating, or participating in recreational fishing. A good way to quantify this goal might be using the number of people on the island at peak tourism (ie population density), with a reference point representing the maximum number of people that could sustainably visit the island at once. Jamie Afflerbach did something similar with OHI+ Northeast to represent beach trampling pressure. 
...
Each resilience category is composed of 1 or more data layers (Table 3.12) with values scaled from 0-1, reflecting the magnitude of resilience. Each resilience data layer is assigned a weight of 0.5 or 1 (Table 3.12) that is applied equally across all the goals (or, goal elements) influenced by the resilience layer (i.e., resilience matrix, Table 3.13). This information is used to calculate a score for each resilience category. The weight reflects information about governance. 


**Table 3.12. Resilience categories and weights**
The data layers used to calculate resilience for Tetiaroa.  Each data layer is assigned to an ecological or social category.  The ecological category is broken into an ecosystem and regulatory category type.
...
**Figure 3.3. Heatmap ranking data inputs for OHI+ Tetiaroa Assessment**
...
In addition to identifying ways existing data could be improved upon, we also identified various data gaps, which if filled would improve the accuracy of results. We list ideas for what types of datasets and research would be most ideal to use for any future assessments below: 

- Water sampling to inform water quality and identify pollution sources
- Tracking hotel occupancy, visitor rates, or some other measure of demand at The Brando
- Detailed maps of island vegetation, updated annually or as frequent as possible
- Detailed benthic habitats maps or coral surveys, updated annually or as frequent as possible
- Localized studies measuring changes in ocean acifidication, sea surface temperature, and sea level rise
- Fishing logs for both recreational activity and artisanal fishing
- Fish stock assessment, repeated annually to detect changes
- Any other habitat services provided by native vegetation, salt water ponds, cyanobacteria mats, etc. 

We think building a strong data sharing partnership with The Brando will help with any future assessments by gaining more insight into potential sources for pollution and other population pressures on the island, since tourism is the main reason humans are visiting Tetiaroa and using its ocean resources. Furthermore, strengthening data sharing agreements and protocols with visiting researchers will help future analysts access more of the data that has been collected on Tetiaroa. This may also help encourage repetition of studies, and improve the accuracy of future OHI assessments by providing evidence of how each goal has changed over time. 

## Alignment with Palmyra

The Palmyra and Tetiaroa projects aligned methods and reference points where possible. Specifically, the Species, Iconic Species, and Coastal Protection goals used the same methods. However, island attributes, management priorities, and data availability made alignment across the entire assessment unfeasible. The Palmyra assessment included a Research goal which was not included in the Tetiaroa assessment. Due to differences in available data, methods for the following goals are not the same across the two assessments: Tourism and Recreation, and Habitats (both coral and native forest). The methods for Lasting Special Places differed as well, due to differences in management goals for each islands' stakeholders. The Clean Waters and Resource Access Opportunities goals were not calcuated for the Tetiaroa assessment due to lack of avialable data, and the Carbon Storage goal was not calculated for either assessment.
...
1. AVISO. (2018). Satellite alimetry data: Mean sea level rise. Satellite alimetry satelite altimetry data. URL https://www.aviso.altimetry.fr/en/data/products/ocean-indicators-products/mean-sea-level.html [accessed 1 August 2020]
2. Boudjelas, S. (2020). BIOSECURITY PLAN FOR TETIAROA ATOLL, SOCIETY ISLANDS, FRENCH POLYNESIA. 
3. Bruno, J.F. & Selig, E.R. (2007). Regional decline of coral cover in the indo-pacific: Timing, extent, and subregional comparisons (R. Freckleton, Ed.). PLoS ONE, 2, e711.
4. Feely, R., Doney, S. & Cooley, S. (2009). Ocean acidification: Present conditions and future changes in a high-CO2 world. Oceanography, 22, 36–47.
5. Fritz, S., See, L., Carlson, T. et al. Citizen science and the United Nations Sustainable Development Goals. Nat Sustain 2, 922–930 (2019). https://doi.org/10.1038/s41893-019-0390-3
6. IUCN. (2016). IUCN red list categories and criteria (version 2016-3). IUCN, Gland, Switzerland.
7. IUCN. (2019a). Spatial data - IUCN red list of threatened species. IUCN red list of threatened species. URL https://www.iucnredlist.org/en [accessed 22 April 2019]
8. IUCN. (2019b). The IUCN red list of threatened species. Version 2019-2. URL http://www.iucnredlist.org/ [accessed 15 May 2019]
9. Jari Hovila, A.A. (2013). OMUVBd: OMI/aura surface UVB irradiance and erythemal dose daily l3 global gridded 1.0 degree x 1.0 degree v3. NASA goddard space flight center, goddard earth sciences data and information services center (GES DISC). URL http://disc.sci.gsfc.nasa.gov/datacollection/OMUVBd_V003.html [Accessed 22 June, 2020]
10. LOF (2017). French-Polynesia-Final-Report-ENGLISH.pdf. (n.d.). Living Oceans Foundation
11. Longo, C. S., Frazier, M., Doney, S. C., Rheuban, J. E., Humberstone, J. M., & Halpern, B. S. (2017). Using the Ocean Health Index to Identify Opportunities and Challenges to Improving Southern Ocean Ecosystem Health. Frontiers in Marine Science, 4. https://doi.org/10.3389/fmars.2017.00020
12. Mcintosh, E., & Malhi, Y. (2018). THE FORESTS OF TETIAROA. 26.
13. Pacific Beachcomber and The Brando Resort (2019). Sustainability and ESG Report 2019.
14. Port Autonome Pepeete (2020) Stopover Handbook in French Polynesia for Mariners.
15. Russell, J. C., Faulquier, L., & Tonione, M. A. (n.d.). Rat invasion of Tetiaroa Atoll, French Polynesia. 6.
16. Saha, Korak; Zhao, Xuepeng; Zhang, Huai-min; Casey, Kenneth S.; Zhang, Dexin; Zhang, Yongsheng; Baker-Yeboah, Sheekela; Relph, John M.; Krishnan, Ajay; Ryan, Thomas (2018). The Coral Reef Temperature Anomaly Database (CoRTAD) Version 6 - 17. Global, 4 km Sea Surface Temperature and Related Thermal Stress Metrics for 1982 to 2017. NOAA National Centers for Environmental Information. Dataset. https://doi.org/10.25921/ffw7-cs39. [Accessed 23 July, 2020] 
17. Samaniego, A. (2019). Understanding the Risks Posed by Rat Reproductive Activity and Diet to Rat Eradication Success on Tropical Islands
18. Sherley, G. (Ed.). (2000). Invasive species in the Pacific: A technical review and draft regional strategy. South Pacific Regional Environment Programme.
19. Tallis, H.T., Ricketts, T., Guerry, A., Wood, S., Sharp, R., Nelson, E., Ennaanay, D., Wolny, S., Olwero, N., Vigerstol, K. & others. (2011). InVEST 2.2.1 user’s guide. The Natural Capital Project, Stanford University.
20. The Tetiaroa Society (2019) CASUP Exotic Species Management and Biosecurity Plan
21. The Tetiaroa Society (2015). CASUP Vegetation & FloraTetiaroa JYM April 2015.docx.
22. The Tetiaroa Society (2016). The Tetiaroa Conservation and Sustainable Use Plan.
23. TRC. (2004). Inventory of coastal areas of local or regional significance in the taranaki region. Taranaki Regional Council, Stratford.
