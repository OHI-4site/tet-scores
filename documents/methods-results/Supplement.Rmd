...
After compiling a list of species from this source, we pulled its conservation status using the IUCN RRedlist package and then assign each species a score based on its conservation status. The overall score is determined by finding the average score per species class, and then taking the overall average among the classes. The table below shows how each IUCN conservation status is scored: 
